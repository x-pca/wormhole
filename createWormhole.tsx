...
import BaseWormhole from './Wormhole';
import { WormholeProps } from './Wormhole';
...
const buildRequestOpenUri = ({
  ...
  try {
    ...
    if (typeof data.jsx !== 'string') {
      throw new Error(`[Wormhole]: Expected string data, encountered ${typeof data.jsx}.`);
    }
    if (await verify(result) !== true) {
      throw new Error(`[Wormhole]: Failed to verify "${uri}".`);
    }

    const component = await shouldCreateComponent(data.jsx);

    Object.assign(cache, { [uri]: { component, styles: data.styles } });
...
}) => (uri: string, callback: PromiseCallback<WormholeCacheData>): void => {
  const { [uri]: compData } = cache;
  const { resolve, reject } = callback;


  // if cached return resolved component
  if (compData === null) {
    return reject(
      new Error(`[Wormhole]: Component at uri "${uri}" could not be instantiated.`)
    );

  } else if (compData && typeof compData.component === 'function') {
...
const buildOpenWormhole = ({
  ...
  readonly shouldOpenUri: (
    uri: string,
    callback: PromiseCallback<WormholeCacheData>
  ) => void;
}) => async (source: WormholeSource, options: WormholeOptions): Promise<WormholeCacheData> => {
  ...
  if (typeof source === 'string') {
    ...
    if (typeof uri === 'string') {
      return new Promise<WormholeCacheData>(
...
export default function createWormhole({
  buildRequestForUri = (config: AxiosRequestConfig) => axios<WormholeSourceData>(config),
